...
(define (expand_TPP current_state)
    (stream (multiply T1 current_state) (multiply T2 current_state) (multiply T3 current_state)))


(define (lazy-breadth-search initial expand)
        ; functie recursiva pentru generarea streamului
        (let search ([states (stream-cons initial empty-stream)])
          (cond [(stream-empty? states) states]

               ; definire stare curenta si restul starilor
               [(let ([current_state (stream-first states)]
                      [rest_states (stream-rest states)])

                      ; formare stream: stare curenta + recursive_call
                      (stream-cons current_state (search (stream-append rest_states (expand current_state)))))])))
                           
(define ppt-stream-in-tree-order
         (lazy-breadth-search '(3 4 5) expand_TPP))
...
(define (pairs G H)
  (let generate ([Gs G] [Hs H] [target 1][it 1])
       (let ([elemG (stream-first Gs)]
             [elemH (stream-first Hs)]
             [restG (stream-rest Gs)]
             [restH (stream-rest Hs)])
             (cond [(= it target) (stream-cons (cons elemG elemH) (generate G restH (add1 target) 1))]
                   [(< it target) (stream-cons (cons elemG elemH) (generate restG Hs target (add1 it)))]))))
...
; naturals fluxul numerelor naturale
; G fluxul numerelor impare
; H fluxul numerelor impare ce incep de la 3
; Se genereaza fluxul numerelor ce nu sunt prime intre ele => cmmdc(x, y) = 1
(define (naturals-from n)
     (stream-cons n (naturals-from (+ n 1))))

(define gh-pairs-stream
  (let* ([naturals (naturals-from 0)] [G (stream-filter odd? naturals)] [H (stream-rest G)])
    (stream-filter (λ (pair) (equal? 1 (gcd (car pair) (cdr pair)))) (pairs G H))))
...
(define ppt-stream-in-pair-order
 (let generate_TPP([pairs-stream gh-pairs-stream])
      (let* ([pair (stream-first pairs-stream)] [g (car pair)] [h (cdr pair)])
        (stream-cons (list (* g h) (/ (* (- h g) (+ h g)) 2) (/ (+ (expt h 2) (expt g 2)) 2)) (generate_TPP (stream-rest pairs-stream))))))
